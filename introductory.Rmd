...
I like a little more flexibility in the file write process, so I usually use the `write.table` function. This can be used to achieve the same end depending on options, but allows other options like omitting `row.names` from the output file. Here is an example of how it works:
```{r, eval=F}
...
Now that we have a handle on how R works, and how to get our data in and back out, we'll spend some time to re-inforce data manipulation and wrap things up with some plotting in [Introductory-II](Introductory-2.html).
